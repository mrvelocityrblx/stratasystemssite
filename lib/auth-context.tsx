...
import type { CredentialUser } from "@/lib/credential-auth"
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = (nextUser: CredentialUser, account: UserAccount) => { setRemainingGenerations(getRemainingGenerations(nextUser.uid)); setMonthlyLimit(getMonthlyLimit(account)); setSubscriptionActive(isSubscriptionActive(account)); setSubscriptionDaysRemaining(getSubscriptionDaysRemaining(account)) }
